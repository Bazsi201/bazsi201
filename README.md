<!--
    Hi, I’m Balázs (Bazsi201)
    Thanks for checking out my GitHub profile.
-->

<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=0:1e3a8a,100:3b82f6&height=120&section=header" />
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&size=24&pause=800&color=3b82f6&width=700&lines=Hi%2C+I'm+Balázs+Mihály;Programming+%26+Data+Science+MSc+Student;Automation+%7C+Power+Platform+%7C+Data" />
</a>

---



### About me
I’m a **Programming & Data Science MSc student** with hands-on experience in **automation, data engineering, and business-critical systems**.  
I enjoy building **robust workflows**, **clean data pipelines**, and **practical tools** that actually get used.

- Strong in **SQL, BigQuery, Power Platform**
- Background in **C# / .NET** and **backend-oriented thinking**
- Interested in **automation, data quality, and scalable systems**
- Fast learner, pragmatic, and detail-oriented

---

### Main skills
[![My Skills](https://skillicons.dev/icons?i=sql,python,cs,dotnet,git,github,azure,js,nodejs,html,css,bootstrap,angular,mysql,postgres)](https://skillicons.dev)

---

### Data & Automation
- **BigQuery** (advanced joins, window functions, data standardization)
- **Power Automate / Power Apps** (complex business flows, Excel & SharePoint automation)
- **SQL optimization & validation**
- **ETL-style data processing**

---

### Currently learning
[![Learning](https://skillicons.dev/icons?i=aws,react)](https://skillicons.dev)

- Advanced data engineering patterns  
- Cloud-based architectures  
- Frontend refresh (Angular / React)

---

### Selected interests
- Data-driven decision making  
- Business process automation  
- Software quality & maintainability  
- AI-assisted productivity

---

### Connect with me
<div>
  <a href="https://www.linkedin.com/in/balazsmihaly/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" width="35"/>
  </a>
</div>

---

### Recruiters / Hiring managers
> Practical mindset, strong data foundations, and real-world automation experience.  
> Open to **junior–medior software, data, or automation roles**.

<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=0:1e3a8a,100:3b82f6&height=120&section=footer" />
